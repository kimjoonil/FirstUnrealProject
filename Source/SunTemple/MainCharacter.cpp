...
AMainCharacter::AMainCharacter()
{
	...
	// 카메라 팔 생성 (충돌시 플레이어 쪽으로 당겨짐)
	CameraBoom = CreateDefaultSubobject<USpringArmComponent>(TEXT("CameraBoom"));
	CameraBoom->SetupAttachment(GetRootComponent());
	CameraBoom->TargetArmLength = 600.f; // 카메라와 플레이어 거리
	CameraBoom->bUsePawnControlRotation = true; // 카메라 회전 컨트롤러
	...
	// 카메라 팔 끝에 카메라를 고정시키고 컨트롤러 방향에 따라 조정되게함
...
void AMainCharacter::Tick(float DeltaTime)
{
	...
	case EStaminaStatus::ESS_Normal:
		...
		else // 쉬프트 키에서 손을 떼었을 떄 일반상태 혹은 손으로 누르고있다 떼었을 때
		{
